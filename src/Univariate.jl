...
function NumericSummary(INPUT::DataArray,VarNames::Symbol)  #数値型の基本統計量の算出
  SummaryVar=var(INPUT)
  SummaryStd=std(INPUT)
  SummaryQuartile=quantile( INPUT , [0.00, 0.25, 0.50, 0.75, 1.00])
  SummaryMedian=median(INPUT)
  SummaryMean=mean(INPUT)
...
function SummaryMerge(INPUT::DataFrame, Summary::DataFrame ,FirstMergeFlag::Bool)  #データの集約
  ...
  return Dict( {"Summary","FirstMergeFlag"},{Summary,FirstMergeFlag})
end
...
function Univariate(INPUT)
  ...
  return Dict({"SummaryNum","SummaryNonNum"},{SummaryNum ,SummaryNonNum})
end
